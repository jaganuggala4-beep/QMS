...
export default function AdminSearch() {
  ...
  const handleSearch = () => {
    ...
    const r = AuditDB.searchByReference(query.trim());
    setResults(r);
    setSearched(true);
  };
  ...
  return (
    <Layout>
      <PageHeader title="Search Audits" subtitle="Find audits by reference number" />
        ...
        <div className="flex gap-3">
          <div className="relative flex-1">
            ...
            <input
              ...
              placeholder="Enter reference number (e.g., REF-20240301-001)"
              className="w-full pl-12 pr-10 py-3 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/30"
            />
            {query && (
          ...
          <button onClick={handleSearch}
            className="bg-blue-600 text-white px-6 py-3 rounded-xl text-sm font-medium hover:bg-blue-700 transition-colors flex items-center gap-2">
                  ...
                  <div className="flex items-center justify-between">
                    <div>
                      <div className="flex items-center gap-2 mb-1">
                        <span className="font-mono font-bold text-blue-600">{audit.referenceNumber}</span>
                    ...
                    <div className="flex items-center gap-3">
                      <div className="text-center">
                        ...
                        <p className="text-xs text-gray-400 mt-0.5">{audit.totalScore}/{audit.maxPossibleScore}</p>
                      </div>
                    ...
                    <div className="mt-4 pt-4 border-t border-gray-100">
                      <div className="grid grid-cols-2 gap-4">
                        <div>
                          ...
                          {audit.scores.map(s => (
                            <div key={s.parameterId} className="flex justify-between items-center py-1">
                              <span className="text-xs text-gray-600">{s.parameterName}</span>
                              <div className="flex items-center gap-1">
                                {s.scoringType === '1-10' ? (
                                  <span className="text-xs font-semibold">{s.givenScore}/10</span>
                                ) : (
                                  <span className={`text-xs font-semibold ${s.givenScore === 'YES' ? 'text-green-600' : s.givenScore === 'NO' ? 'text-red-600' : 'text-gray-400'}`}>
                                    {s.givenScore as string}
                                  </span>
                                )}
                                <span className="text-xs text-gray-400">• {s.weightage}%</span>
                              </div>
                            </div>
                          ))}
                        </div>
                        <div>
                          ...
                          <p className="text-xs text-gray-600 bg-gray-50 rounded-lg p-3">{audit.feedback || 'No feedback'}</p>
                        </div>
                      </div>
                    </div>
                  )}
